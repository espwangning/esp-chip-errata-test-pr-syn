...
def evaluate_condition(condition, target):
    # return condition == self.target_name  # Example condition check
    return condition == target if target is not None else False


class Entry:
    """Tags to pages map"""

    def __init__(self, entrypath: Path, idf_target: str):
        ...
        for line in self.lines:
            ...
            if line.startswith(".. only::"):
                # Extract the condition from the only directive
                only_condition = line.split(".. only::")[1].strip()
                continue
            # If there's a condition, evaluate it
            if only_condition and not evaluate_condition(only_condition, self.idf_target):
                continue  # Skip further processing if the condition is not met
...
def assign_entries(app):
    ...
    for path in doc_paths:
        entry = Entry(Path(app.srcdir) / path, app.config.idf_target)
